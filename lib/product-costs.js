...
function calculate(product,base,inventory=[]){
 ...
 const filmCost=film.calculate(f.lamina,units,inventory);
 if(filmCost){if(filmCost.reason)missing.push(filmCost.reason);rows.push({...filmCost,group:'Lámina del inventario',name:'Lámina: '+filmCost.name,amount:filmCost.amount==null?null:round(filmCost.amount)});}
 for(const [key,label] of Object.entries(extraLabels)){
  ...
  const rowLabel=key==='empaques'&&filmCost?'Otros empaques (sin la lámina seleccionada)':label;
  if(!known)missing.push('Completar '+rowLabel.toLowerCase()+' por presentación (0 si no aplica).');
  rows.push({group:'Adicionales del producto',name:rowLabel,amount:known?Number(value):null});
 }
...
async function report(pool,userId){
 ...
 try{
  ...
  const rows=products.map(p=>calculate(p,bases.rows.find(b=>b.id===p.formula_id),bases.inventory));
