...
const Reconciliation=require('./order-reconciliation');
...
function register(app,pool,verifyToken){
  ...
  app.get('/api/pedidos/:id/finanzas',verifyToken,async(req,res)=>{
    try{await allowed(pool,req.user.userId);const row=(await pool.query('SELECT * FROM pedidos WHERE numero_pedido=$1',[req.params.id])).rows[0];if(!row)throw error(404,'Pedido no encontrado.');res.set('Cache-Control','no-store').json({pedido:row,token:token(row),summary:Core.summary(row),correction:Reconciliation.describe(row)});}
  ...
  app.post('/api/pedidos/:id/ajustes-financieros',verifyToken,async(req,res)=>{
    ...
    try{
      ...
      const actor={id:user.id,name:user.name||user.username,role:user.role},now=new Date().toISOString();
      if(b.type==='REVERSO'&&row.financial_data?.events?.some(e=>e.id===b.target&&e.subtype==='CONCILIACION_PEDIDO'))throw error(400,'Para corregir una conciliación, registra otra conciliación con los datos correctos.');
      let data,corrected;
      try{
        if(b.type==='CONCILIAR_PEDIDO'){
          const catalog=await client.query('SELECT nombre FROM catalogo');
          const names=new Set([...(row.products||[]).map(p=>p.name),...catalog.rows.map(p=>p.nombre)]);
          if(!Array.isArray(b.lines)||b.lines.some(l=>!names.has(typeof l.name==='string'?l.name.trim():null)))throw Error('Selecciona productos del catálogo o del pedido actual.');
          corrected=Reconciliation.reconcile(row,b,actor,now);data=corrected.financial_data;
        }
        else data=Core.append(row,b,actor,now);
      }catch(e){throw error(400,e.message);} 
      data.events[data.events.length-1].requestHash=requestHash;
      const saved=corrected?
        (await client.query('UPDATE pedidos SET financial_data=$1::jsonb, products=$2::jsonb, entregas=$3::jsonb, producto=$4, cantidad=$5, subtotal=$6, history=$7::jsonb, pago=$8::jsonb WHERE numero_pedido=$9 RETURNING *',[JSON.stringify(data),JSON.stringify(corrected.products),JSON.stringify(corrected.entregas||[]),corrected.producto,corrected.cantidad,corrected.subtotal,JSON.stringify(corrected.history),JSON.stringify(corrected.pago||null),req.params.id])).rows[0]:
        (await client.query('UPDATE pedidos SET financial_data=$1::jsonb WHERE numero_pedido=$2 RETURNING *',[JSON.stringify(data),req.params.id])).rows[0];
