...
function recalculate(item,rows,next){
  ...
  // A saved pre-movement valuation or an empty opening balance is an independent
  // starting point. Replay from it rather than subtracting a price difference
  // from a current average that may already be wrong (or previously rounded).
  const recordedCost=first.cost_before?.cost;
  const hasOpeningCost=recordedCost!=null&&Number.isFinite(Number(recordedCost))&&Number(recordedCost)>=0;
  const emptyOpening=first.before===0&&next.tipo==='ingreso';
  if(sameQty&&old.tipo==='ingreso'&&!hasOpeningCost&&!emptyOpening){
    ...
    if(!Number.isFinite(cost)||cost < -0.000001)throw fail(409,'El promedio actual no coincide con el precio histórico y falta el costo del saldo anterior para reconstruirlo. Se necesita revisar la valoración anterior a este ingreso; no se ha guardado ningún cambio.');
    return {stock,cost:Math.max(0,cost)};
  }
  // Quantity/type changes require the original valuation, not a current-price substitute.
  let initialCost;
  if(emptyOpening)initialCost=0;
  else if(hasOpeningCost)initialCost=Number(recordedCost);
  else{
  ...
  if(!Number.isFinite(cost)||cost<0)throw fail(409,'La valoración reconstruida no es válida. No se ha guardado ningún cambio.');
