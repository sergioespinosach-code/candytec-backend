...
async function record(pool,userId,itemId,body){
  ...
  try{
    ...
    let nextCost=purchase?(stock*cost+input.cantidad*input.precio)/next:item.costo_prom;
    let basis=item.cost_basis||null;
    if(purchase&&basis?.method==='purchase_weighted'){
      const entries=(await client.query('SELECT * FROM movimientos WHERE item_id=$1 ORDER BY id',[input.item_id])).rows;
      const average=purchaseAverage.calculate([...entries,input]);
      nextCost=average.cost;basis=average.basis;
    }
    const updated=(await client.query('UPDATE inventario SET stock=$1, costo_prom=$2, cost_basis=$4::jsonb WHERE id=$3 RETURNING *',[next,nextCost,input.item_id,JSON.stringify(basis)])).rows[0];
    const movement=(await client.query('INSERT INTO movimientos (item_id,tipo,cantidad,motivo,usuario,fecha,operation_key,operation_payload,precio,cost_before) VALUES ($1,$2,$3,$4,$5,NOW(),$6,$7::jsonb,$8,$9::jsonb) RETURNING *',[input.item_id,input.tipo,input.cantidad,input.motivo,(user.name||user.username)+' · '+user.role,input.operation_key,JSON.stringify(payload),purchase?input.precio:null,JSON.stringify({stock,cost})])).rows[0];
