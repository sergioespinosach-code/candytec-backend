...
async function resources(db){const inventory=(await db.query("SELECT id,nombre,unidad,tipo,costo_prom FROM inventario WHERE tipo='materia_prima' ORDER BY nombre")).rows;const products=(await db.query('SELECT id,nombre FROM catalogo ORDER BY nombre')).rows;return {inventory,products};}
