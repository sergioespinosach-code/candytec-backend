...
function validate(body) {
  ...
  const lamina=film.validate(body.lamina);
  if(lamina!==undefined)ficha.lamina=lamina;
  if(body.costos_adicionales!==undefined){
    const extras=body.costos_adicionales;
    if(!extras||typeof extras!=='object'||Array.isArray(extras))throw core.fail('Costos adicionales inválidos.');
    ficha.costos_adicionales={};
    for(const key of ['empaques','componentes','proceso']){
      const v=extras[key];
      if(v==null||v===''){ficha.costos_adicionales[key]=null;continue;}
      if(!['string','number'].includes(typeof v)||!Number.isFinite(Number(v))||Number(v)<0||Number(v)>1e7)throw core.fail('Costo adicional inválido: '+key);
      ficha.costos_adicionales[key]=Number(v);
    }
  }
...
async function save(pool,userId,body) {
  ...
  try {
    ...
    if (action==='save') {
      ...
      if(data.ficha.lamina){
        const item=(await db.query('SELECT id,nombre,unidad,tipo,costo_prom FROM inventario WHERE id=$1 FOR SHARE',[data.ficha.lamina.item_id])).rows[0];
        if(!item)throw core.fail('La lámina seleccionada no existe. Actualiza la ficha.');
        const cost=film.priceKg(item);
        data.ficha.lamina={...data.ficha.lamina,nombre:item.nombre,costo_kg_referencia:cost};
      }else if(data.ficha.lamina===undefined&&before?.ficha?.lamina!==undefined){data.ficha.lamina=before.ficha.lamina;}
      if(before&&data.ficha.costos_adicionales===undefined&&before.ficha?.costos_adicionales!==undefined)data.ficha.costos_adicionales=before.ficha.costos_adicionales;
      if(!!before?.ficha?.lamina!==!!data.ficha.lamina&&body.data?.costos_adicionales===undefined&&data.ficha.costos_adicionales)data.ficha.costos_adicionales={...data.ficha.costos_adicionales,empaques:null};
      if (id) {
      ...
      if (action==='archive') {
        const items=(await db.query("SELECT * FROM inventario WHERE tipo='producto_terminado' AND nombre=$1 ORDER BY id FOR UPDATE",[before.nombre])).rows;
        for(const item of items){
          const stock=Number(item.stock);
          if(!Number.isFinite(stock))throw core.fail('El saldo del producto no es válido.',409);
          if(stock===0)continue;
          const movement=(await db.query('INSERT INTO movimientos (item_id,tipo,cantidad,motivo,usuario,fecha) VALUES ($1,$2,$3,$4,$5,NOW()) RETURNING *',[item.id,stock>0?'ajuste_baja':'ajuste_alza',Math.abs(stock),'Baja de existencias por eliminación del portafolio: '+before.nombre,(user.name||user.username)+' · Gerencia'])).rows[0];
          const cleared=(await db.query('UPDATE inventario SET stock=0 WHERE id=$1 RETURNING *',[item.id])).rows[0];
          await db.query('INSERT INTO business_audit(entity,entity_id,actor_id,actor_name,reason,before_data,after_data) VALUES($1,$2,$3,$4,$5,$6::jsonb,$7::jsonb)', ['inventory',item.id,user.id,user.name||user.username,'Eliminar producto del portafolio',JSON.stringify(item),JSON.stringify({...cleared,movement_id:movement.id,product_id:id})]);
        }
      }
...
function register(app,pool,verifyToken) {
  ...
  app.get('/api/product-master',verifyToken,route(async req=>{
    ...
    return {rows:rows.map(p=>{const base=bases.rows.find(f=>f.id===p.formula_id);return {...p,formulation_name:base?.data.name||null,mass_cost:formulations.massCost(p,base)};}),categories:CATEGORIES,formulations:bases.rows,films:film.choices(bases.inventory)};
  }));
