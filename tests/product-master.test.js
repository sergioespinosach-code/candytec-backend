...
function dbFixture(role='gerente'){
 const s={materials:[{id:17,nombre:'Lámina YumYum',tipo:'materia_prima',unidad:'kg',costo_prom:6}],formulas:[{id:7}],catalog:[],inventory:[],movements:[],audit:[],ops:[],role,fail:false};let backup;
 ...
 if(sql.startsWith('INSERT INTO inventario')){if(!s.inventory.some(x=>x.nombre===p[0]))s.inventory.push({id:s.inventory.length+1,tipo:'producto_terminado',nombre:p[0],stock:0});return result([]);}
 ...
 if(sql.startsWith("SELECT * FROM inventario WHERE tipo='producto_terminado'"))return result(s.inventory.filter(x=>x.nombre===p[0]&&x.tipo==='producto_terminado'));
 if(sql.startsWith('INSERT INTO movimientos')){const m={id:s.movements.length+1,item_id:p[0],tipo:p[1],cantidad:p[2],motivo:p[3]};s.movements.push(m);return result([m]);}
 if(sql.startsWith('UPDATE inventario SET stock=0')){const row=s.inventory.find(x=>x.id===p[0]);row.stock=0;return result([row]);}
...
test('eliminar da de baja existencias una vez y restaurar conserva saldo cero',async()=>{
 const db=dbFixture();await master.save(db,1,body());db.s.inventory[0].stock=10;
 const b=body({id:1,version:1,action:'archive',operation_key:'product-operation-000002'});
 const archived=await master.save(db,1,b);assert.equal(archived.activo,false);assert.equal(db.s.inventory[0].stock,0);
 assert.equal(db.s.movements[0].cantidad,10);assert.equal(db.s.movements[0].tipo,'ajuste_baja');
 await master.save(db,1,b);assert.equal(db.s.movements.length,1);
 const restored=await master.save(db,1,body({id:1,version:2,action:'restore',operation_key:'product-operation-000003'}));
 assert.equal(restored.activo,true);assert.equal(restored.id,1);assert.equal(db.s.inventory[0].stock,0);
});
test('fallo al eliminar revierte simultáneamente baja y archivo, sin afectar materias primas',async()=>{
 const db=dbFixture();await master.save(db,1,body());db.s.inventory[0].stock=12;
 db.s.inventory.push({id:2,nombre:data().nombre,tipo:'materia_prima',stock:99});db.s.fail=true;
 await assert.rejects(master.save(db,1,body({id:1,version:1,action:'archive',operation_key:'product-operation-000002'})),/Fallo/);
 assert.equal(db.s.catalog[0].activo,true);assert.equal(db.s.inventory[0].stock,12);assert.equal(db.s.movements.length,0);
 db.s.fail=false;await master.save(db,1,body({id:1,version:1,action:'archive',operation_key:'product-operation-000003'}));assert.equal(db.s.inventory[1].stock,99);
});
test('eliminar sin saldo no crea movimientos; permite limpiar existencias de un archivado anterior',async()=>{
 const db=dbFixture();await master.save(db,1,body());await master.save(db,1,body({id:1,version:1,action:'archive',operation_key:'product-operation-000002'}));
 assert.equal(db.s.movements.length,0);db.s.inventory[0].stock=8;
 await master.save(db,1,body({id:1,version:2,action:'archive',operation_key:'product-operation-000003'}));assert.equal(db.s.inventory[0].stock,0);assert.equal(db.s.catalog[0].activo,false);
});
