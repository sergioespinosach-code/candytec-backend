...
async function seed() {
  ...
  try {
    ...
    if (old.rows.length) {
      await pool.query("UPDATE users SET username = 'produccion', role = 'produccion' WHERE username = 'jefe'");
      console.log("— Usuario 'jefe' renombrado a 'produccion'\n");
    }
    // Reparación: cuentas que ya se renombraron antes de este fix (username correcto, pero role todavía en 'jefe')
    const fix = await pool.query("SELECT id FROM users WHERE username = 'produccion' AND role != 'produccion'");
    if (fix.rows.length) {
      await pool.query("UPDATE users SET role = 'produccion' WHERE username = 'produccion' AND role != 'produccion'");
      console.log("— Rol interno de 'produccion' reparado (estaba desincronizado)\n");
    }
